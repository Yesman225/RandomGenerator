...
CallTopBoundary::CallTopBoundary(double sMax, double strike, double rate) : R1R1Function(), SMax(sMax), Strike(strike), Rate(rate)
{}

double CallTopBoundary::operator()(double t)
{
	return std::max(SMax - Strike*std::exp(-Rate*t), 0.);
}

PutTopBoundary::PutTopBoundary(double sMax, double strike, double rate) : R1R1Function(), SMax(sMax), Strike(strike), Rate(rate)
{}

double PutTopBoundary::operator()(double t)
{
	return std::max(Strike - SMax*std::exp(-Rate*t), 0.);
}
